...
class SudokuCell:
	def __init__(self, grid, y, x):
		...
		block_start_y = y/3 * 3
		block_start_x = x/3 * 3
		self.block = [(block_start_y + m_y, block_start_x + m_x) for m_y in range(3) for m_x in range(3)]
		self.excluded = set()
	...
	def update_exclusions(self, grid):
		if not self.value:
			...
			if len(self.excluded) == 8:
				self.value = list(set(range(1,10)).difference(self.excluded))[0]
	...
	def group_necessity(self, grid, group):
		...
		for cell_location in group:
			if cell_location != (self.y, self.x):
				...
				interseciton_of_exlcusions.intersection_update(other_cell.excluded)
...
class Sudoku:
	def __init__(self, grid):
		# print grid
		self.cells = [[SudokuCell(grid, y, x) for x in range(9)] for y in range(9)]
		# print self.cells
	...
	def check_solved(self):
		...
		for block_y in range(3):
			for block_x in range(3):
				block_values = [raw_grid[3*block_y + m_y][3*block_x + m_x] for m_y in range(3) for m_x in range(3)]
